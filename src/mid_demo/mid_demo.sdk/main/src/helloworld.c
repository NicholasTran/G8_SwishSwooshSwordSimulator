...
float degreesFromHorizontalBlock(NAV_RectCoord r) {
   ...
   if (rM == 0)
      return 0.0;
   return acosf(r.Y / rM) * (180.0 / M_PI);
}
	...
	while(1){//
		switch(gameState)
		{
				...
				while(1 && (DEBUG_MODE==0)){
					...
					*(kb_base+0) = 1;
					...
					if (keyboard == KB_MODE_1){
						...
						playerHealth = 3;
						enemyHealth = 3;
						playerPeriod = PLAYER_ACTION_PERIOD;
						printf("Calling srand: %d", waitCounter);
						srand(waitCounter);
						gameState = STATE_DRAW_PROMPT;
					...
					// Hard (?) Mode
						...
						playerHealth = 1;
						enemyHealth = 9;
						playerPeriod = PLAYER_ACTION_PERIOD/2;
						printf("Calling srand: %d", waitCounter);
						srand(waitCounter);
						gameState = STATE_DRAW_PROMPT;
						break;
			...
			case STATE_DRAW_PROMPT:
				actionCounter += rand()%3 + 1;
				DrawHearts(playerHealth, enemyHealth, hardmode);
				DrawGtrack(0,0,0);
				DrawTimer(0, playerPeriod);
			...
			case STATE_WAIT_FOR_ACTION:
				...
				while(waitCounter < playerPeriod)
				{
					NAV_GetDeviceID(&nav);
					NAV_GetData(&nav);
					...
					DrawTimer(waitCounter, playerPeriod);
				...
				if(foundCorrectAction) {
					...
					{
						score += 1;
						if(playerPeriod - waitCounter <= PARRY_WINDOW)
						{
			...
			case STATE_FREE_HIT:
				printf("Perfect Parry! You get a free hit!");
				waitCounter = 0;
